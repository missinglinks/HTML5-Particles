...
function ParticleField(field_width, field_height, pSize, a) {
	...
	this.populate = function(particleNumber)
	{
		...
		for (var i = 0; i < particleNumber; i++)
		{
			...
			{
				next = true;
				var newX = Math.round(this.size+Math.random()*(this.fieldWidth-this.size*2));
				var newY = Math.round(this.size+Math.random()*(this.fieldHeight-this.size*2));
				
				for (var j = 0; j < i; j++)
				{
					var abstand = this.checkDistance(this.particles[j], new Particle(newX,newY));
	...
	this.populateText = function(pText, pTextSize,textX, textY, colorScheme, resizeOff)
	{
		this.particles = new Array();
		this.text = pText;
		this.initText = pText;
		...
		canvasBuffer.width = this.fieldWidth*3;
		canvasBuffer.height = this.fieldHeight;
		var buffer = canvasBuffer.getContext('2d');
		buffer.fillStyle = "rgb(0,0,0)";
		buffer.fillRect(0,0,this.fieldWidth*3, this.fieldHeight);
		
		if (!colorScheme)
			colorScheme = "bright";
		
		
		buffer.font = this.textSize+"px Helvetica";
		...
		if (!resizeOff)
		{
			...
			{
				this.textSize = Math.round(this.textSize * ((this.fieldWidth-20)/textWidth));
				buffer.font = this.textSize+"px Helvetica";
		...
		var bufferTextData = buffer.getImageData(0, 0, textWidth, this.textSize*1.5).data; 
		...
		if (!textX)
			var xOffset = Math.round((this.fieldWidth/2)-(textWidth/2));
		else 
			var xOffset = textX;
		if (!textY)
		...
		for (var i = 0; i < bufferTextData.length; i++) 
		{
			...
			if (((i+1)%4 != 0) && (Math.floor((row)%((this.size+1)*4)) == 1) && (line%(this.size+1) == 1))
			{
				//alert(skipRows);
				if (bufferTextData[i] != 0)
				{
					this.particles.push(new Particle(
						Math.floor(row/4)+ xOffset,
						line+yOffset));
					if (colorScheme == "dark")
	...
	this.checkIntegrity = function(deleteNotVisible, deleteNaN)
	{
		...
		for (var i = 0; i< this.particles.length; i++)
		{
			...
			if (deleteNotVisible)
			{
				...
				if ((this.particles[i].x < 0) || (this.particles[i].x > this.fieldWidth) ||
					(this.particles[i].y < 0 || this.particles[i].y > this.fieldHeight))
					this.particles.splice(i,1);
				
			}
			if (deleteNaN && !del)
			{
				if (isNaN(this.particles[i].x) ||isNaN(this.particles[i].y) || isNaN(this.particles[i].moveX) || isNaN(this.particles[i].moveY))
				{
					this.particles.splice(i, 1);
					//alert("del");
				}
	...
	this.collide = function(particle1, particle2, abstand)
	{
		...
		if(isNaN(deltaX) || isNaN(deltaY) || isNaN(skalar1)) alert('NaN delta  '+skalar1+' '+deltaX+' '+deltaY+' ');
	...
	this.updateQuadrants = function()
	{
		...
		for (i = 0; i < this.particles.length; i++)
		{
			...
			if(((qI >= 0) && (qI < this.quadrants.length)) && ((qJ >= 0) && (qJ < this.quadrants[0].length)))
	...
	this.update = function(modifier) {
		...
		for (var i = 0; i < this.particles.length; i++)
		{
			...
			if (this.particles[i].glow > 0) 
				this.particles[i].glow -= modifier*1.5;
			
			
						
			var mX = this.particles[i].moveX*this.particles[i].moveDistance*modifier*5;
			var mY = this.particles[i].moveY*this.particles[i].moveDistance*modifier*5;			
			var collided=false;
			...
			{	
				...
				if ((qI < this.quadrants.length-1) && (qJ > 0))
					collArray =collArray.concat(this.quadrants[qI+1][qJ-1]);
				if ((qI < this.quadrants.length-1) && (qJ < this.quadrants[0].length-1))
					collArray =collArray.concat(this.quadrants[qI+1][qJ+1]);
				...
				for(var j=0; j<collArray.length; j++)
				{
					if(collArray[j]!=i)
					{
						...
						var dis = this.checkDistance(new Particle(this.particles[i].x+mX, this.particles[i].y+mY),
													this.particles[collArray[j]]);
						if (dis < this.size+1) 
						{
							this.collide(this.particles[i],this.particles[collArray[j]],this.size);		
							collided = true;		
						}
					}
				}
			...
			{
				this.particles[i].x += this.particles[i].moveX*this.particles[i].moveDistance*modifier*5;
				if ((this.particles[i].x < this.size/2) || 
					(this.particles[i].x > this.fieldWidth-this.size/2))
				{ 
					if (this.particles[i].x < this.size/2) this.particles[i].x = this.size/2;
				...
				{
					if (this.particles[i].y < this.size/2) this.particles[i].y = this.size/2;
					else this.particles[i].y = this.fieldHeight-this.size/2;
				 		this.particles[i].moveY = -this.particles[i].moveY;
				}
				this.particles[i].moveDistance -= this.particles[i].moveDistance*modifier;
		...
		for (var q = 0; q<this.quadrants[0].length; q++)
		{
			...
			for (i = 0; i < this.particles.length; i++)
			{
				if ((this.particles[i].x < this.fieldWidth) && (this.particles[i].x > 0))
				{
					...
					{				
						if ((Math.round(this.particles[i].x) != Math.round(this.particles[i].path[0].x)) ||
							(Math.round(this.particles[i].y) != Math.round(this.particles[i].path[0].y)))
						{
							cvx.strokeStyle="rgba("+this.particles[i].rgbString+",0.2)";
							cvx.beginPath();
							cvx.lineWidth = 1;
							cvx.moveTo(this.particles[i].path[0].x, this.particles[i].path[0].y+this.size/2);
					...
					cvx.fillStyle="rgba("+this.particles[i].rgbString+","+this.alpha+")";
		    		cvx.beginPath();
		    		cvx.arc(this.particles[i].x,		    			
		    				this.particles[i].y, 
		    				this.size/2, 
		    				0, 
		    				Math.PI*2, 
